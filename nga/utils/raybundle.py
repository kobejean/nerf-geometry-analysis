...
def cube_eval_ray_bundle(dataparser_outputs, sampling_depth, dimensions=(1.,1.,1.), n = 1001):
    ...
    for axis in range(3):
        ...
        axis_fars = torch.full((2*n, n, 1), 2*max_depth * dataparser_scale)
    ...
    pixel_area = (dataparser_scale ** 2) * torch.ones((2*n, 3*n, 1)) / (n ** 2)
...
def sphere_eval_ray_bundle(dataparser_outputs, sampling_depth, radius=0.5, n = 1001, m = 2001):
    ...
    pixel_area = (dataparser_scale ** 2) * torch.ones((n, m, 1)) / (n * m)
